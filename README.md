...
### RAG 电力调度应用（第三章）
- **知识库管理** — 文档上传、文本提取、清洗脱敏、分块、向量化入库（ChromaDB）
- **调度助手** — 混合检索（BM25 + 稠密向量）、CrossEncoder 重排、通义千问生成回答
- **决策审计** — 问答日志记录、关键词检索、分类筛选、CSV 导出

### 安全攻防验证平台（第六章）
- **环境准备** — RAG 环境装载、知识库初始化、模型资产管理
- **攻击验证** — 知识库投毒攻击模拟与效果验证
- **防御配置** — 输入/检索/输出三级护栏策略
- **态势感知** — 多源事件采集、攻击链关联、风险评估
- **评测复盘** — 指标统计、过程回放、报告生成
