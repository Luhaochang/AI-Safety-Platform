...
import {
  ...
  ZoomEvent, MoveEvent, Line,ILeaferConfig
} from 'leafer-ui'
...
class MarkCanvas {
  ...
  translate(x: number, y: number) {
    // 平移标注图层
    this.objectsLayer.zoomLayer.x = x
    this.objectsLayer.zoomLayer.y = y
  ...
  appZooming(e: ZoomEvent) {
    ...
    // 平移画布
    LeafHelper.zoomOfWorld(this.objectsLayer, center, scale)
    // 修正坐标
    this.scale(this.objectsLayer.zoomLayer.scaleX!)
    this.translate(this.objectsLayer.zoomLayer.x!, this.objectsLayer.zoomLayer.y!)
  }
  /** 平移事件 */
  appMoving(e: MoveEvent) {
    if (e.buttons == 0) return
    LeafHelper.moveWorld(this.objectsLayer, e.moveX, e.moveY)
    this.scale(this.objectsLayer.zoomLayer.scaleX!)
    this.translate(this.objectsLayer.zoomLayer.x!, this.objectsLayer.zoomLayer.y!)
