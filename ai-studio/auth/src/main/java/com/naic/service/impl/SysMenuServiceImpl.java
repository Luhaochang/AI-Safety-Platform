...
import com.naic.domain.entity.cnv.SysMenuCnv;
...
import com.naic.domain.entity.vo.MetaVO;
import com.naic.domain.entity.vo.RouterVO;
import com.naic.mapper.SysMenuMapper;
...
public class SysMenuServiceImpl extends ServiceImpl<SysMenuMapper, SysMenuPO> implements ISysMenuService
{
    ...
    private final SysMenuCnv sysMenuCnv;
    ...
    public List<SysMenuVO> selectMenuList(SysMenuDTO menu, Long userId)
    {
        ...
        else
        {
            ...
            menuList = menuMapper.selectMenuListByUserId(menu, LoginIdHolder.getLoginId());
        }
        for(SysMenuPO po:menuList){
            voList.add(sysMenuCnv.poToVo(po));
        }
        return voList;
    }
    ...
    {
        return sysMenuCnv.poToVo(menuMapper.selectById(menuId));
    }
    ...
    public int insertMenu(SysMenuDTO menu)
    {
        ...
        return menuMapper.insert(sysMenuCnv.dtoToPo(menu));
    }
    ...
    {
        return menuMapper.updateById(sysMenuCnv.dtoToPo(menu));
    }
    ...
    public boolean checkMenuNameUnique(SysMenuDTO menu)
    {
        ...
        List<SysMenuPO> list=menuMapper.selectList(new QueryWrapper<SysMenuPO>().eq("menu_name",menu.getMenuName()).eq("parent_id",menu.getParentId()));
        if (list.isEmpty()){
            return UserConstants.UNIQUE;
        }
        SysMenuPO info = list.get(0);
